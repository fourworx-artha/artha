...
- (2026-06-11) `ENGINE_DEFAULTS` shipped early (W6 STEP 0, pulled forward by audit fix A1) — `calculatePayslip` resolves `{ ...ENGINE_DEFAULTS, ...familyConfig }`; numeric stage-gated keys default to 0 so configs missing them can never produce NaN. The session-24 interim `streakBonusEnabled: true` was retired in session 25: it is now `false` per spec, safe because `migrateStageConfig` backfills pre-W6 families' member.config
- (2026-06-11) W6 stage system shipped — stage is per-child and derived (`deriveStage(settledCount, stageOverride)` in `src/utils/stages.js`), never stored; pure derivation/patch helpers in `utils/stages.js`, stage data (STAGES/STAGE_PATCHES/FEATURE_STAGES) in `utils/constants.js`; `applyStagePatches` (operations.js) is the single write path for settle advancement, "Skip the guided period", and addMember-under-override; `configTouched` lists (one per config layer) protect parent edits from patches
- (2026-06-11) Pre-W6 families self-migrate via `migrateStageConfig` (one-shot, FamilyContext load): copies each child's EFFECTIVE stage-key values (family.config first, patch defaults fallback) into member.config for unlocked keys, then strips the 4 stage-gated keys from family.config — behaviour-preserving for Economist children
- (2026-06-11) `payslips.stage` column stamps the stage a payslip was earned at (draft time); PayslipCard REMOVES (not dims) rows above that stage; legacy NULL renders full. Migration: `docs/migrations/W6_payslips_stage.sql` — must run in Supabase before any post-W6 payslip run
- (2026-06-11) `settlePayslip` returns `{ settled, stageAdvanced }` — advancement detection runs after the RPC, never inside it, and never fails the settle; W9 celebrations consume `stageAdvanced`
